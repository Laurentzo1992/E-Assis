...
const DashboardPage = () => {
  ...
  const handleSuccess = (newCompany) => {
    // Met à jour l'entreprise active, recharge la liste et ferme le modal
    (async () => {
      try {
        // Essayer de définir l'entreprise créée comme active côté serveur
        const setActiveRes = await apiRequest(
          "entreprise/entreprises/set-active/",
          {
            method: "POST",
        ...
        if (setActiveRes.ok) {
          const activeData = await setActiveRes.json();
          setActiveCompany(activeData);
        } else {
          // Si le serveur ne peut pas définir l'active, on applique quand même côté client
          setActiveCompany(newCompany);
        }

        if (typeof refetchUserCompanies === "function") {
          await refetchUserCompanies(); // rafraîchit la liste affichée dans la sidebar
        }
        showCustomAlert("Entreprise créée avec succès !", "success");
      } catch (err) {
        console.error("Erreur lors du set-active après création :", err);
        // Fallback : définir la company localement
        setActiveCompany(newCompany);
        if (typeof refetchUserCompanies === "function") {
          refetchUserCompanies();
        }
        showCustomAlert("Entreprise créée (localement).", "success");
      } finally {
  ...
  const renderContent = () => {
    if (loadingActiveCompanyData) {
      return (
        <Spinner message="Chargement des informations de l'entreprise..." />
      );
    }
      ...
      return (
        <div className="text-center p-5 bg-light rounded m-4">
          ...
          <p className="text-muted">
            Pour commencer, veuillez créer votre première entreprise.
          </p>
          <button
    ...
    switch (activeSection) {
      ...
      case "profil":
        return (
          <ProfileSection {...companyData} showCustomAlert={showCustomAlert} />
        );
      case "alertes":
      default:
        return (
  ...
  return (
    <>
      <div className="dashboard-container">
        ...
        <main className="main-wrapper">{renderContent()}</main>
