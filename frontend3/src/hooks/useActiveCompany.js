...
const useActiveCompany = (showCustomAlert) => {
  ...
  const [profileData, setProfileData] = useState({
    nom: "", numeroIdentification: "", siret: "", adresse: "",
    email: "", telephone: "", nomRepresentant: "", prenomRepresentant: "",
    secteurActivite: null, domainesActivite: [],
  });
  ...
  const refetchUserCompanies = useCallback(async () => {
    ...
    try {
      ...
      const data = await response.json();
      setUserCompanies(data.results || data);
    } catch (err) {
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      try {
        ...
        const activeCompanyRes = await apiRequest("entreprise/entreprises/active/");
        if (activeCompanyRes.ok) {
          ...
          setProfileData({
            ...
            secteurActivite:
              activeCompanyData.secteurs && activeCompanyData.secteurs.length > 0
          ...
          if (userCompanies.length === 0) { // userCompanies est à jour grâce à refetchUserCompanies
            ...
            throw new Error(`Erreur HTTP pour l'entreprise active: ${activeCompanyRes.status} - ${JSON.stringify(errorData)}`);
        }
      } catch (err) {
  ...
  return {
    userCompanies, setUserCompanies,
    activeCompany, setActiveCompany,
    loading, error,
    showAddEntrepriseModal, setShowAddEntrepriseModal,
    profileData, setProfileData,
    apiDomainesActivite, loadingDomaines, errorDomaines, fetchDomaines,
    apiSecteursActivite, loadingSecteurs, errorSecteurs, fetchSecteurs,
